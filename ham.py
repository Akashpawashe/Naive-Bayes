...
sms = pd.read_csv("D:\\excelR\\Data science notes\\Naive bayes\\asgmnt\\sms_raw_NB.csv",encoding = "ISO-8859-1")
sms.head()
sms.shape
sms.info()
sms.isnull().sum() # No missing values
sms.drop_duplicates(keep='first', inplace=True) # 603 rows are duplicates
sms.dtypes
...
import re
stop_words = []
with open("D:/excelR/Data science notes/stop.txt") as f:
    stop_words = f.read()
...
cleaning_text("hope i can understand this your feelings 123121. 123 hi how .. are you?")
...
email_train,email_test = train_test_split(sms,test_size=0.3)


# Preparing email texts into word count matrix format 
emails_bow = CountVectorizer(analyzer=split_into_words).fit(sms.text)
...
# For all messages
all_emails_matrix = emails_bow.transform(sms.text)
